...
int NeedDriftCorrect() {
  int nResult = 0;
  long idTemplate = g_pExperiment->GetCurrentTrial()->m_pItemCell->GetTemplate()->GetID();
  long idBlock = g_pExperiment->GetCurrentTrial()->m_idBlock;

  g_pErr->Debug(pastestr::paste("sdsd", " ", "NeedDC: template", idTemplate, "block", idBlock));

  if (dcFlag || (!(curtrialix % 12)) ) { // drift correct every 12 trials
    nResult = 1;
    dcFlag = 0;
  } else {}
  ...
  // drift correct for the next trial
  if ( idTemplate == 2  ) {
    dcFlag = 1;
  } else {}
...
int single_trial() {
  ...
  eyecmd_printf("draw_box %d %d %d %d 7", 183, 482, 383, 682);
  eyecmd_printf("draw_box %d %d %d %d 7", 412,  86, 612, 286);
  eyecmd_printf("draw_box %d %d %d %d 7", 641, 482, 841, 682);
...
int app_main(char * trackerip, DISPLAYINFO * disp)
{
  ...
  */
  ...
  // GetFileName will iterate for each "run" of a single session
  strcpy(our_file_name, g_pExperiment->GetFileName().c_str());
...
int parseArgs(int argc, char **argv, char **trackerip, DISPLAYINFO *disp )
{
  ...
  for( i =1; i < argc; i++)
      ...
      else if (_stricmp(argv[i],"-session") ==0 && argv[i+1]) 
	{
      ...
      //else if (_stricmp(argv[i],"-trial") ==0 && argv[i+1]) 
      //{
	  //i++;
	  //g_nStartTrial = atoi(argv[i]);
      //}
      else if (_stricmp(argv[i],"-resume") ==0)
	{
	  g_bResume = true;
	  dcFlag = 1; // drift correct first trial when resuming
	}
      else
	{
	  printf("%d \n", i);
